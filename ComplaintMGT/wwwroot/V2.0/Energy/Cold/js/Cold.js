...
$('#imgApplyFilter').click(function () {
    ...
    $.each(allSelectedGraph, function (key, value) {
        ...
        //clear all indicator
        if (enableGraph == true) {
            var divIndicator = '<button type="button" class="' + activeClass + '" data-bs-target="#carouselEnergyTrendColdIndicators" style="background-color:black;" data-bs-slide-to="' + key + '" aria-label="Slide ' + (key + 1) + '"></button>'
            ...
            if (frequencyEnable == true) {
                $.getJSON('/v2.0/Energy/Hot/js/EnergyConsumption_Graph_Config.json', function (data) {
                    ...
                    if (selectedGraphData) {
                        ...
                        //initialize Graph
                        energyColdDashboard.init(graphId, graphContainerID);
                    }
                })
